...
$(document).ready(function () {
    ...
    $('#cost').click(function () {
        var height = parseFloat($('#height').val());
        var width = parseFloat($('#width').val());
        var costm2 = parseFloat($('#productCost').text().trim());
        var area = parseFloat((height / 100 * width / 100) / 100).toFixed(2);
        ratio = $('#productСolor.form-control').val().toLowerCase() == '#ffffff' ? 0 : 80;
        var pokraska = ratio * area;
        // var result = area * cost;
        var result = parseFloat((parseInt(height, 10) / 100 * parseInt(width, 10) / 100) * typePercent * costm2 / 100 + pokraska).toFixed(0);
